...
await queue?.textChannel?.send({ content: `Пользователи покинули канал! | 🔴 Users left channel!!` }).catch(e => { })
if(queue || queue?.playing){
...
await queue?.textChannel?.send({ content: `Микрофон отключен! |🔴 Muted` }).catch(e => { })
}
}
