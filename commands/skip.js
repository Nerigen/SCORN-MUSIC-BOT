  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      if (number) {
        ...
        if (isNaN(number)) return interaction.reply({ content: '⚠️Некорректное число! | ⚠️ Invalid Number', ephemeral: true }).catch(e => { })
        if (1 > number) return interaction.reply({ content: '⚠️Некорректное число! | ⚠️ Invalid Number', ephemeral: true }).catch(e => { })
...
try {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(success ? ` **SKIPPED** : **${old.name}**` : '❌Очередь пуста! | ❌ Queue is empty!')
    .setTimestamp();

  return interaction.reply({ embeds: [embed] });
}catch (e) {
          return interaction.reply({ content: '❌Очередь пуста! | ❌ Queue is empty!!', ephemeral: true }).catch(e => { })
        }
      }
